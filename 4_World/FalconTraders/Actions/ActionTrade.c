...
{		
	...
	override void OnExecuteServer( ActionData action_data )
	{
		...
		if (FT_HumanityHelpers.checkIfPlayerHasEnoughtHumanity(player.GetIdentity().GetId(), trader))
		{
		...
		GetRPCManager().SendRPC("FalconTraders", "FTmessagePlayerC", new Param1<string>(FT_HumanityHelpers.getHumanityMessage(player.GetIdentity().GetId(), trader)), true,  player.GetIdentity());
	}
